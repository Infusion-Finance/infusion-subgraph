...
export function handleBurn(event: Burn): void {
  ...
  // update the LP position
  let liquidityPosition: LiquidityPosition;
  if (burn!.sender === null) {
    liquidityPosition = createLiquidityPosition(event.address, Address.fromString('0x0000000000000000000000000000000000000000'));
  } else {
    liquidityPosition = createLiquidityPosition(event.address, Address.fromString(burn!.sender!.toHexString()));
  }
  createLiquiditySnapshot(liquidityPosition, event)
